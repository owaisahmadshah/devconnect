...
import type { LikeService } from '../services/like.service.js';
...
export class LikeController {
   ...
   */
  createLike = asyncHandler(async (req: Request, res: Response) => {
    if (!req.user) {
      throw new ApiError(HttpStatus.UNAUTHORIZED, 'Authenticated user not found in request');
    }

    const data: TCreateLike = req.body;
    const result = await this.service.createLike(data, req.user._id);

    return res
      .status(HttpStatus.OK)
      .json(new ApiResponse(HttpStatus.OK, result, 'Reaction toggled successfully.'));
  });
}
